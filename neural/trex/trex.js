...
jump_neurons = Array(hidden_count);
run_neurons = Array(hidden_count);
...
function init() {
	...
	for (var k = 0; k< hidden_count; k++) {
		jump_neurons[k] = new Neuron(hidden_count + 1);
		run_neurons[k] = new Neuron(hidden_count + 1);
	}
	for (var j = 0; j < history_length; j++) {
		...
		canvas.height = height;
		var context = canvas.getContext("2d");
		hidden_context[hidden_count+j+1]= context;
		hidden_canvas[hidden_count+j+1] = canvas;
	}
}
	...
	if (!r.crashed) { //} && !r.paused && r.playing) {
		...
		*/
		...
		for (var i = 0; i < hidden_count/2; i++) {
			...
			var l_jump_score = forward(vector, l_jump_neuron, hidden_neurons);
			var l_run_score = forward(vector, l_run_neuron, hidden_neurons);
		...
		if (history_vector.length > history_length) {
			...
			if (action == 1) {
				//debugger;
				train(v[0], error, neuron, hidden_neurons);
			} else {
				train(v[0], error, neuron, hidden_neurons);
			}
		}
